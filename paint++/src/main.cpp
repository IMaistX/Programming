        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::KeyPressed) {
                switch (event.key.code) {
                    ...
                    case sf::Keyboard::Equal:
                        if (brushRadius < 20) ++brushRadius; // TODO: увеличить brushRadius на 1, но не более 20
                        break;
                    case sf::Keyboard::Hyphen:
                    case sf::Keyboard::Subtract:
                        if (brushRadius > 1) --brushRadius; // TODO: уменьшить brushRadius на 1, но не менее 1
                        break;
            ...
            if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left) {
                ...
                if (mouse.y >= paletteY && mouse.y <= paletteY + 30) {
                    if (mouse.x >= 10) {
                        int idx = (mouse.x - 10) / 35;
                        if (idx >= 0 && idx < palette.size() && (mouse.x - 10) % 35 < 30) {
                            selectedPaletteIndex = idx;
                            currentColor = palette[idx];
                        }
                    }
